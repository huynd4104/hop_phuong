...
class _BackupScreenState extends ConsumerState<BackupScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      padding: EdgeInsets.fromLTRB(MediaQuery.sizeOf(context).width < 600 ? 16 : 24, 8, MediaQuery.sizeOf(context).width < 600 ? 16 : 24, 24),
      child: ref.watch(appRepositoryProvider).when(
        loading: () => const Center(child: CircularProgressIndicator()),
        error: (error, _) => ErrorView(message: error.toString(), onRetry: () => ref.invalidate(appRepositoryProvider)),
          ...
          child: Column(
            ...
            children: [
              FeatureCard(
                ...
                subtitle: 'Xuất toàn bộ cơ sở dữ liệu để sao lưu ngoại tuyến hoặc khôi phục từ ảnh chụp trước đó.',
                trailing: FilledButton.icon(
                  onPressed: () async {
                    try {
                      final file = await BackupService(repository).exportBackup();
                      if (context.mounted) {
                        showSnackBar(context, 'Đã lưu sao lưu: ${file.path}');
                      }
                    } catch (error) {
                      if (context.mounted) {
              ...
              FeatureCard(
                ...
                subtitle: 'Chọn một tệp sao lưu và thay thế cơ sở dữ liệu cục bộ bằng nội dung của tệp đó.',
                  ...
                  onPressed: () async {
                    try {
                      ...
                      if (kIsWeb) {
                        ...
                        if (path == null) return;
                        await BackupService(repository).importBackupFromFile(File(path));
                      }

                      refreshAll(ref);
                      if (context.mounted) {
                        showSnackBar(context, 'Đã khôi phục dữ liệu thành công');
                      }
                    } catch (error) {
                      if (context.mounted) {
                        showSnackBar(context, error.toString(), isError: true);
              ...
              FeatureCard(
                ...
                subtitle: 'Tạo tệp .xlsx cho tháng được chọn trên tab Bảng kê.',
                trailing: FilledButton.tonalIcon(
                  onPressed: () async {
                    try {
                      final rows = await repository.buildMonthlyStatement(month: filter.month, year: filter.year, query: '');
                      final file = await StatementExportService().exportMonthlyStatementToExcel(month: filter.month, year: filter.year, statements: rows);
                      if (context.mounted) {
                        showSnackBar(context, 'Đã lưu file Excel: ${file.path}');
                      }
                    } catch (error) {
                      if (context.mounted) {
                        showSnackBar(context, error.toString(), isError: true);
              ...
              FeatureCard(
                ...
                subtitle: 'Xóa cơ sở dữ liệu cục bộ và nạp lại dữ liệu thử nghiệm để kiểm tra.',
